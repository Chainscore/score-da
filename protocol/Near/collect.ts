 ...
 * that both throughput and cost plotters consume.
 *
 * Outputs:
 *   analysis/blocks.csv       — per-block data (same schema as old throughput.csv)
 *   analysis/prices.csv       — NEAR/USD hourly prices (90d from CoinGecko)
 *   analysis/chain_config.json — protocol config + gas cost breakdown
 *
 * Usage:
 *   npx tsx collect.ts                            # last 1 day (RPC)
 *   npx tsx collect.ts --days 90                  # 90 days (auto: NEAR Lake S3)
 *   npx tsx collect.ts --source lake --days 7     # force lake source
 *   npx tsx collect.ts --source rpc --blocks 5000 # force RPC source
 *   npx tsx collect.ts --rpc url1,url2,url3       # custom RPCs
 */
...
async function main(): Promise<void> {
  ...
  // ---- Write all outputs ----
  await saveBlocks(allRows);
  console.log(`CSV    -> analysis/blocks.csv  (${allRows.length} rows)`);

  if (prices.length > 0) {
    await savePrices(prices);
    console.log(`CSV    -> analysis/prices.csv  (${prices.length} rows)`);
  }

  await saveConfig(cc, gasConfig, MILESTONES, startHeight, endHeight, prices);
  console.log(`Config -> analysis/chain_config.json`);
