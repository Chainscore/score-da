"""Celestia DA — Generate research paper figures from daily.csv."""
...
def main():
    ...
    save(plot_data_volume(
        d, df["daily_blob_mib"], df["daily_blob_mib_7d"],
        f"{PROTO} — Daily Blob Volume (MiB)",
    ), OUT_DIR, "data_volume")
    ...
    save(plot_cumulative(
        d, df["cumulative_daily_blob_mib"], df["cumulative_daily_da_spend_usd"],
