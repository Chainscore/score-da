 ...
 *   sales.csv           — per-sale-cycle data
 *   purchases.csv       — individual core purchases with price paid
 *   renewals.csv        — individual core renewals with price paid
 *   ondemand.csv        — on-demand orders (relay chain)
 *   dot_prices.csv      — historical DOT/USD daily prices
 *   broker_config.json  — broker pallet configuration snapshot
 *   cost_config.json    — derived cost metrics + DOT/USD
 *
 * Usage:
 *   npx tsx data/cost/collect.ts                           # default
 *   npx tsx data/cost/collect.ts --ondemand-blocks 5000    # relay scan window
 *   npx tsx data/cost/collect.ts --max-pages 50            # more Subscan pages (100 events/page)
 */
